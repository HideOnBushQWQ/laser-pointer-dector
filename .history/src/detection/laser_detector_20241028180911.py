...
class LaserDetector:
    ...
    def detect_laser(self, frame):
        ...
        # Define red color range for center detection
    ...
    def check_outer_region(self, hsv, x, y, w, h):
        ...
        outer_layers = [
            {"lower": np.array([0, 100, 100]), "upper": np.array([10, 200, 200]), "dilation": 2},
            {"lower": np.array([0, 50, 50]), "upper": np.array([10, 150, 150]), "dilation": 4},
        ]
        ...
        for layer in outer_layers:
            ...
            # Require at least 20% red pixels in the outer layer to confirm
            if red_pixel_ratio < 0.2:
                return False
