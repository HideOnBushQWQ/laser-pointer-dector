...
class LaserDetector:
    ...
    def detect(self, frame):
        ...
        # Convert to HSV color space for red halo detection
        hsv = cv2.cvtColor(blurred, cv2.COLOR_BGR2HSV)
        red_mask = cv2.inRange(hsv, self.red_lower, self.red_upper)

        # Convert to grayscale for high-intensity center detection
        gray = cv2.cvtColor(blurred, cv2.COLOR_BGR2GRAY)
        _, bright_center_mask = cv2.threshold(gray, 250, 255, cv2.THRESH_BINARY)  # High threshold for bright center

        # Combine red halo and bright center masks
        combined_mask = cv2.bitwise_and(red_mask, bright_center_mask)
